...
Deep3DDescriptor::Deep3DDescriptor()
    : cloud_{new IntensityCloud},
      keypoints_{new IntensityCloud},
      selected_keypoints_{new IntensityCloud} {}

void Deep3DDescriptor::setInputCloud(const IntensityCloud::Ptr &cloud_input) {
  cloud_ = cloud_input;
}
void Deep3DDescriptor::setRadius(const float nb_radius_input) {
  nb_radius_ = nb_radius_input;
}
void Deep3DDescriptor::setKeypoints(
    const IntensityCloud::Ptr &keypoints_input) {
  keypoints_ = keypoints_input;
}
IntensityCloud Deep3DDescriptor::getSelectedKeypoints() {
  return (*selected_keypoints_);
}
void Deep3DDescriptor::getImagePatch(
...
#pragma omp parallel for
  for (size_t p = 0; p < keypoints_->points.size(); ++p) {
    ...
    Eigen::Vector4f min_vec_bbox(min_x, min_y, min_z, 1);
    Eigen::Vector4f max_vec_bbox(max_x, max_y, max_z, 1);
    const float max_depth{max_vec_bbox.lpNorm<2>()};
    const float min_depth{min_vec_bbox.lpNorm<2>()};
    const float step_size{nb_radius_ / kPatchSize};
    std::vector<int> indices;
    pcl::getPointsInBox(*cloud_, min_vec_bbox, max_vec_bbox, indices);
      ...
      for (size_t k = 1; k <= kPatchSize; k++) {
        ...
        for (auto &index : indices_bbox) {
          Eigen::Vector3f point_vec =
              cloud_slice.points[index].getVector3fMap();
          avg_depth += point_vec.lpNorm<2>();
          pcl::PointXYZI point_slice =
              cloud_->points[indices[indices_slice[index]]];
        ...
        image_patch.at<cv::Vec2f>(63 - (i - 1), 63 - (k - 1))[0] =
            avg_intensity;
  ...
  for (auto &image_patch : vecD_initial) {
    if (image_patch.rows == 0)  /// the keypoint didn't have a patch
    {
      ...
      cv::Mat image_patch_float(kPatchSize, kPatchSize, CV_64FC2,
                                cv::Scalar(0.0));
...
void Deep3DDescriptor::compute(DescriptorCloud &features) {
  ...
  std::cout << "Input keypoint size: " << keypoints_->points.size()
            << std::endl;
  ...
  std::cout << "out of " << keypoints_->points.size()
            << " keypoints, patches computed for "
            << selected_keypoints_->points.size() << " keypoints" << std::endl;
  ...
  transport->open();
  size_t ctr{0};
  std::vector<double> patch_vector(selected_keypoints_->points.size() *
                                   kPatchSize * kPatchSize * 2);
