...
int main(int argc, char **argv) {
  ...
  if (pcl::console::find_argument(argc, argv, "--path_to_source_pcd_file") >= 0)
    input_path_source = argv[pcl::console::find_argument(
                                 argc, argv, "--path_to_source_pcd_file") +
                             1];
  else {
  ...
  if (pcl::console::find_argument(argc, argv, "--sampling_radius_source") >= 0)
    sampling_radius_source =
        atof(argv[pcl::console::find_argument(argc, argv,
                                              "--sampling_radius_source") +
                  1]);
  else {
  ...
  if (pcl::console::find_argument(argc, argv, "--path_to_target_pcd_file") >= 0)
    input_path_target = argv[pcl::console::find_argument(
                                 argc, argv, "--path_to_target_pcd_file") +
                             1];
  else {
  ...
  if (pcl::console::find_argument(argc, argv,
                                  "--feature_neighborhood_radius") >= 0)
  ...
  if (pcl::console::find_argument(argc, argv, "--use_learned_metric") >= 0)
    use_metric = atoi(
        argv[pcl::console::find_argument(argc, argv, "--use_learned_metric") +
             1]);
  else {
  ...
  if (pcl::console::find_argument(argc, argv, "--use_ransac") >= 0)
    use_ransac =
        atoi(argv[pcl::console::find_argument(argc, argv, "--use_ransac") + 1]);
  else {
  ...
  std::cout << "first cloud loaded with " << input_cloud_source->points.size()
            << " number of points" << std::endl;
  if (input_cloud_source->points.empty()) {
    cerr << "path might be wrong because the first pointcloud is empty"
         << std::endl;
    return (1);
  }
  ...
  std::cout << "target cloud loaded with " << input_cloud_target->points.size()
            << " number of points" << std::endl;

  if (input_cloud_target->points.empty()) {
    cerr << "path might be wrong because the second pointcloud is empty"
         << std::endl;
    return (1);
  }
  ...
  //////finding keypoints using uniform sampling. Can be replaced by any
  ///keypoint detector
  ...
  std::cout << "number of keypoins for cloud 1: "
            << keypoints_source->points.size() << std::endl;
  ...
  std::cout << "number of keypoints for cloud 2: "
            << keypoints_target->points.size() << std::endl;
  ...
  IntensityCloud selected_keypoints_source =
      estimate_deep_descriptor.getSelectedKeypoints();
  ...
  IntensityCloud selected_keypoints_target =
      estimate_deep_descriptor.getSelectedKeypoints();
  ...
  if (use_metric == 1) {
    ...
    for (size_t index_source = 0;
         index_source < deep_descriptors_source.points.size(); ++index_source) {
      float min_distance = std::numeric_limits<float>::max();
      int min_index = -1;
      for (size_t index_target = 0;
           index_target < deep_descriptors_target.points.size();
           ++index_target) {
        float distance = pcl::L2_Norm(
            deep_descriptors_source.points[index_source].descriptor,
            deep_descriptors_target.points[index_target].descriptor,
            kDescriptorSize);
...
/*  boost::shared_ptr<pcl::visualization::PCLVisualizer> viewer_correspondence(*/
  ...
  pcl::visualization::PointCloudColorHandlerCustom<IntensityPoint> green(
      input_cloud_source, 0, 102, 0);
  //pcl::visualization::PointCloudColorHandlerCustom<IntensityPoint> purple(
      //input_cloud_target, 51, 0, 102);
  //viewer_correspondence->addPointCloud<IntensityPoint>(
      //input_cloud_source, green, "input_cloud_source");
  //viewer_correspondence->addPointCloud<IntensityPoint>(
      //input_cloud_target, purple, "input_cloud_target");
  //viewer_correspondence->setPointCloudRenderingProperties(
      //pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 6, "input_cloud_source");
  //viewer_correspondence->setPointCloudRenderingProperties(
  ...
  if (use_ransac == 1) {
    ...
    for (auto &corr : inlier_correspondences) {
      ss.str("");
      ss << "line_" << ctr;
  /*    viewer_correspondence->addLine(*/
          //selected_keypoints_source.points[corr.index_query],
          //selected_keypoints_target.points[corr.index_match], 255, 0, 0,
          /*ss.str());*/
      ctr += 1;
    }
    ...
    svd.estimateRigidTransformation(selected_keypoints_source,
  ...
  viewer_alignment->setPointCloudRenderingProperties(
      pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 5, "cloud_aligned");
  viewer_alignment->setPointCloudRenderingProperties(
      pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 5, "input_cloud_target");
  viewer_alignment->setBackgroundColor(255, 255, 255);

  viewer_alignment->setCameraParameters(cam);
