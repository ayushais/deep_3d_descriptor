...
int main(int argc, char **argv) {
  ...
  if (pcl::console::find_argument(argc, argv, "--path_to_pcd_file") >= 0)
    input_path =
        argv[pcl::console::find_argument(argc, argv, "--path_to_pcd_file") + 1];
  else {
  ...
  std::cout << "cloud loaded with " << input_cloud->points.size()
  ...
  ////finding keypoints using uniform sampling. Can be replaced by any keypoint
  ///detector
  ...
  std::cout << "number of keypoins: " << cloud_filter->points.size()
            << std::endl;
  ...
  std::cout << "total number of features computed are: "
            << deep_descriptors.points.size() << std::endl;
}
